...
orientMuitoDir = trapmf(orientacao, [MIN_ORIENT -0.8 -0.6 -0.5]); % muito_direita
orientPoucoDir = trapmf(orientacao, [-0.6 -0.5 -0.1 0]); % pouco_direita
orientReto = trapmf(orientacao, [-0.1 0.01 0.09 0.1]); % reto
orientPoucoEsq = trapmf(orientacao, [0 0.1 0.5 0.6]); % pouco_esquerda
orientMuitoEsq = trapmf(orientacao, [0.5 0.6 0.8 MAX_ORIENT]); % muito_esquerda
 
% Saida: Velocidade Angular (-1 a 1)
omega = MIN_OMEGA:0.1:MAX_OMEGA;
omgMuitoHorario = trimf(omega, [MIN_OMEGA -0.5 -0.2]); % muito_horario
omgPoucoHorario = trimf(omega, [-0.3 -0.15 0]); % pouco_horario
omgZero = trimf(omega, [-0.1 0 0.1]); % zero
omgPoucoAntiHor = trimf(omega, [0 0.15 0.3]); % pouco_antihorario
...
if (sentido == 1)
	...
	R6 = fuzzyficar2 (distClose, orientMuitoEsq, omgPoucoHorario);
	R7 = fuzzyficar2 (distClose, orientPoucoEsq, omgZero);
